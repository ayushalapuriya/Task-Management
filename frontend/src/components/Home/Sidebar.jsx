        ...
        <>
            ...
            <div>
                {data.map((items,i)=>(
                    ...
                    >
                    <span className='text-xl'>{items.icon}</span>&nbsp;{items.title}</Link>
                ))}
            </div>
            <div>
                <button className='bg-gray-600 w-full p-2 rounded hover:bg-gray-700' onClick={logout}>Log Out</button>
            </div>
        </>
    )
}
